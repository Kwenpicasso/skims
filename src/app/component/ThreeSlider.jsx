...
async function getData() {
  const query = `*[_type == 'product'][15...19] | order(_createdAt asc){
  ...
  return (
    ...
    >
      <CarouselContent>
          ...
          {data.map((item) => (
            <CarouselItem key={item._id} className="basis-1/2 w-full ">
            <div className="p-1">
              <Card>
    ...
    <Image src={item.image}  width={300} height={300} alt="three" className=' w-full block' />
    {item.class ? (
      <h1 className='cc capitalize'>{item.class}</h1>
    ) : ''}
    </div>
    <h1 className=' w-[200px] sm:w-[170px] mx-auto flex justify-center items-center uppercase text-center text-darkbrown text-xs font-semibold mt-2'>{item.mainname}</h1>
    <h1 className=' w-[300px] sm:w-[200px] text-darkbrown text-xs font-semibold flex justify-center items-center mt-2'>${item.price}</h1>
      ...
      <CarouselPrevious className='ml-5 sm:ml-7' />
      <CarouselNext className='sm:mr-7 mr-5'/>
    </Carousel>
  )
}
